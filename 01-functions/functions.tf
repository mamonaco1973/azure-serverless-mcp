resource "azurerm_storage_account" "functions" {
  name                     = "costmcpfunc${random_id.suffix.hex}"
...
resource "azurerm_service_plan" "cost_mcp" {
  name                = "cost-mcp-plan"
  resource_group_name = azurerm_resource_group.cost_mcp.name
  location            = azurerm_resource_group.cost_mcp.location
  os_type             = "Linux"
...
resource "azurerm_application_insights" "cost_mcp" {
  name                = "cost-mcp-ai"
  resource_group_name = azurerm_resource_group.cost_mcp.name
  location            = azurerm_resource_group.cost_mcp.location
...
resource "azurerm_function_app_flex_consumption" "cost_mcp" {
  name                = "cost-mcp-func-${random_id.suffix.hex}"
  resource_group_name = azurerm_resource_group.cost_mcp.name
  location            = azurerm_resource_group.cost_mcp.location

  service_plan_id = azurerm_service_plan.cost_mcp.id
  https_only      = true
  ...
  app_settings = {
    ...
    APPLICATIONINSIGHTS_CONNECTION_STRING = azurerm_application_insights.cost_mcp.connection_string
    AzureWebJobsFeatureFlags              = "EnableWorkerIndexing"
    SUBSCRIPTION_ID                       = data.azurerm_client_config.current.subscription_id
    # Used by in-code JWT validation to verify token audience and issuer.
    API_CLIENT_ID                         = azuread_application.cost_mcp_api.client_id
