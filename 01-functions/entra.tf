...
resource "azuread_application" "cost_mcp_api" {
  display_name = "cost-mcp-api"
}

resource "azuread_service_principal" "cost_mcp_api" {
  client_id = azuread_application.cost_mcp_api.client_id
}

# Proxy app registration — the MCP proxy authenticates as this service principal.
resource "azuread_application" "cost_mcp_proxy" {
  display_name = "cost-mcp-proxy"
}

resource "azuread_service_principal" "cost_mcp_proxy" {
  client_id = azuread_application.cost_mcp_proxy.client_id
}

# Client secret for the proxy service principal.
resource "azuread_application_password" "cost_mcp_proxy" {
  application_id = azuread_application.cost_mcp_proxy.id
  display_name   = "cost-mcp-proxy-secret"
